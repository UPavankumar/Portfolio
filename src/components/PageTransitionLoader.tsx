...
export default function PageTransitionLoader() {
  ...
  useLayoutEffect(() => {
    ...
    // Skip short loading shutter on mobile phones (< 768px) where it felt clunky
    if (window.innerWidth < 768 || !ROUTE_INFO[location.pathname]) {
      setLoading(false);
      return;
    }

    // Immediately cover screen on desktop BEFORE browser paint
    setLoading(true);
    setIsRevealing(false);
    setCount(0);

    const duration = 850; // Smooth readable desktop count
